...
for i in range(0,num_leds):
    ...
    if i % 2 == 0:
        # r for frames[0]
        frames[0] = frames[0]+("\\x%02x\\x%02x\\x%02x" % (darken * 255, 0, 0)) #red
        # g for frames[1]
        frames[1] = frames[1]+("\\x%02x\\x%02x\\x%02x" % (0, darken * 255, 0)) #green
    else:
        # g for frames[0]
        frames[0] = frames[0]+("\\x%02x\\x%02x\\x%02x" % (0, darken * 255, 0)) #green
        # r for frames[1]
        frames[1] = frames[1]+("\\x%02x\\x%02x\\x%02x" % (darken * 255, 0, 0)) #red
